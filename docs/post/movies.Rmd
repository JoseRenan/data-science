...
bootstraps = ratings_hp %>% 
  boot(statistic = bootstrap_hp_movies, R = 5000)
```
...
```{r, echo=FALSE}
hp_ci = boot.ci(bootstraps, conf = 0.95, type = "basic")
tibble(estatistica = as.double(bootstraps$t)) %>% 
  ggplot(aes(x = estatistica)) + 
  geom_histogram(binwidth = 0.01, fill = "white", color = "black") +
  geom_vline(xintercept=hp_ci$basic[[4]], colour = "IndianRed") +
  geom_vline(xintercept=hp_ci$basic[[5]], colour = "IndianRed") +
  labs(title = "Distribuição de probabilidade da diferença das médias de avaliações dos filmes\nde Harry Potter antes e depois do 3º filme",
       x = "Avaliações") 
```
...
jp_movies_id = jp_movies %>%
  pull(movieId)

jp_ids_after_second = jp_movies %>%
  filter(year > 2001) %>%
  pull(movieId)

ratings_jp = ratings %>% 
  filter(movieId %in% jp_movies_id)

ratings_jp = ratings_jp %>%
  mutate(afterSecond = (movieId %in% jp_ids_after_second))
```

```{r, include=FALSE}
set.seed(1234)

bootstrap_jp_movies <- function(data, indexes){
    d = data %>% 
      slice(indexes) %>%
      group_by(afterSecond) %>% 
      summarise(meanRating = mean(rating)) %>%
      pull(meanRating)
    return((d[1] - d[2]))
}

bootstraps_jp = ratings_jp %>% 
  boot(statistic = bootstrap_jp_movies, R = 5000)
```
...
```{r, echo=FALSE}
jp_ci = boot.ci(bootstraps_jp, conf = 0.95, type = "basic")
tibble(media_diferenca = as.double(bootstraps_jp$t)) %>% 
  ggplot(aes(x = media_diferenca)) + 
  geom_histogram(binwidth = 0.01, fill = "white", color = "black") +
  geom_vline(xintercept=jp_ci$basic[[4]], colour = "IndianRed") +
  geom_vline(xintercept=jp_ci$basic[[5]], colour = "IndianRed") +
  labs(title = "Distribuição de probabilidade da diferença das médias de avaliações dos filmes\nda saga Jurassic Park antes e depois da continuação",
       x = "Avaliações") 
```
...
```{r, include=FALSE}
bootstrap_hp_movie = function(data, indexes){
    d = data %>% 
      slice(indexes) %>%
      pull(rating)
    return(mean(d))
...
calculate_ci = function(id) {
  ci = ratings_hp %>%
    filter(movieId == id) %>% 
    boot(statistic = bootstrap_hp_movie, R = 2000) %>% 
    boot.ci(conf = 0.95, type = "basic")
  ci
}

hp_movies_cis = hp_movies %>%
  nest(movieId) %>% 
  mutate(ci = map(data, ~ calculate_ci(.x$movieId))) %>%
  mutate(lower = map_dbl(ci, ~ .x$basic[[4]]), upper = map_dbl(ci, ~ .x$basic[[5]])) %>% 
  select(-ci, -data)
```
...
hp_movies_cis %>% 
    ggplot() + 
    aes(x = reorder(year, year), y=((lower + upper) / 2), ymin = lower, ymax = upper) + 
    geom_errorbar(width = .1) + 
    labs(
        title = "IC das médias das avaliações de cada filme da saga Harry Potter",
        x = "Ano do filme", 
        y = "Avaliação"
    )
```
...
```{r, include=FALSE}
bootstrap_jp_movie = function(data, indexes){
    d = data %>% 
      slice(indexes) %>%
      pull(rating)
    return(mean(d))
...
calculate_ci = function(id) {
  ci = ratings_jp %>%
    filter(movieId == id) %>% 
    boot(statistic = bootstrap_jp_movie, R = 2000) %>% 
    boot.ci(conf = 0.95, type = "basic")
  ci
}

jp_movies_cis = jp_movies %>%
  nest(movieId) %>% 
  mutate(ci = map(data, ~ calculate_ci(.x$movieId))) %>%
  mutate(lower = map_dbl(ci, ~ .x$basic[[4]]), upper = map_dbl(ci, ~ .x$basic[[5]])) %>% 
  select(-ci, -data)
...
jp_movies_cis %>% 
    ...
    geom_errorbar(width = .1) + 
    labs(
        title = "IC das médias das avaliações de cada filme da saga Jurassic Park",
        x = "Ano do filme", 
        y = "Avaliação"
    )
```